...
class QwenEditModule(QwenManager):
    ...
    def _prepare_input_image(self, image: Image, megapixels: Optional[float] = None):
        """
        V2: Prepare input image with proper scaling while preserving aspect ratio.
        Supports adaptive resolution based on image complexity.
        ...
        if megapixels is None:
            # V2: Use adaptive megapixels if enabled
            if getattr(self.settings, 'qwen_edit_adaptive_resolution', False):
                from modules.utils import calculate_adaptive_megapixels
                megapixels = calculate_adaptive_megapixels(image, self.settings.qwen_edit_megapixels)
                logger.info(f"Using adaptive megapixels: {megapixels:.2f}MP")
            else:
        ...
        logger.debug(f"Resizing image from {image.width}x{image.height} to {width}x{height} ({megapixels:.2f}MP)")
    ...
    def edit_image(self, prompt_image: Image.Image, seed: int, **kwargs):
        """ 
        V2: Edit the image using Qwen Edit with optional parameter overrides.

        Args:
            prompt_image: The prompt image to edit.
            seed: Random seed for generation
            **kwargs: Optional parameter overrides (for intelligent retry)
        ...
        try:
            ...
            # V2: Allow parameter overrides
            prompting.update(kwargs)
